defmodule Roguelandia.AccountsTest do
  use Roguelandia.DataCase

  alias Roguelandia.Accounts

  import Roguelandia.AccountsFixtures
  alias Roguelandia.Accounts.{User, UserToken}
  ...
  describe "players" do
    ...
    @invalid_attrs %{level: nil, special: nil, hp: nil, strength: nil, attack: nil}
    ...
    test "create_player/1 with valid data creates a player" do
      valid_attrs = %{level: 42, special: "some special", hp: 42, strength: 42, attack: "some attack"}

      assert {:ok, %Player{} = player} = Accounts.create_player(valid_attrs)
      assert player.level == 42
      assert player.special == "some special"
      assert player.hp == 42
      assert player.strength == 42
      assert player.attack == "some attack"
    end
    ...
    test "update_player/2 with valid data updates the player" do
      player = player_fixture()
      update_attrs = %{level: 43, special: "some updated special", hp: 43, strength: 43, attack: "some updated attack"}

      assert {:ok, %Player{} = player} = Accounts.update_player(player, update_attrs)
      assert player.level == 43
      assert player.special == "some updated special"
      assert player.hp == 43
      assert player.strength == 43
      assert player.attack == "some updated attack"
    end
