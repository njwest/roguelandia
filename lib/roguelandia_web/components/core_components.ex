defmodule RoguelandiaWeb.CoreComponents do
  @moduledoc """
  ...
  def flash(assigns) do
    ...
    <div
      ...
      {@rest}
    >
      <p :if={@title} class="flex items-center gap-1.5 font-semibold leading-6 bg-inherit text-inherit">
        <.icon :if={@kind == :info} name="hero-information-circle-mini text-inherit" class="h-4 w-4" />
        <.icon :if={@kind == :error} name="hero-exclamation-circle-mini" class="h-4 w-4 text-inherit" />
        <%= @title %>
      </p>
      <p class="mt-2 leading-5  bg-inherit text-inherit"><%= msg %></p>
  ...
  def button(assigns) do
    ...
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 bg-brand text-white hover:bg-opacity-90 py-2 px-3 text-lg sm:text-2xl bg-brand dialogue-border",
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div>
      ...
      <select
        id={@id}
        name={@name}
        class="sm:text-lg mt-2 block w-full rounded-md border border-gray-300 bg-white shadow-sm focus:border-zinc-400 focus:ring-0"
        multiple={@multiple}
        {@rest}
      >
  ...
  def input(%{type: "textarea"} = assigns) do
    ...
    <div>
      ...
      <textarea
        ...
        name={@name}
        class={[
          "sm:text-lg mt-2 block w-full rounded-lg text-zinc-900 focus:ring-0 sm:sm:leading-6 min-h-[6rem]",
  ...
  def label(assigns) do
    ~H"""
    <label for={@for} class="block sm:text-2xl leading-6">
      <%= render_slot(@inner_block) %>
    </label>
    """
  end
  ...
  def error(assigns) do
    ~H"""
    <p class="mt-3 flex gap-3 leading-6 text-rose-600">
          ...
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-zinc-50">
            ...
            <td :if={@action != []} class="relative w-14 p-0">
              <div class="relative whitespace-nowrap py-4 text-right font-medium">
                ...
                <span
                  :for={action <- @action}
                  class="relative ml-4 font-semibold leading-6  hover:text-gray-200"
                >
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(RoguelandiaWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(RoguelandiaWeb.Gettext, "errors", msg, opts)
    end
  end
